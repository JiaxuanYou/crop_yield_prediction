    ...
    try:
        for i in range(config.train_step):
            ...
            if i%10 == 0:
                ...
                logging.info('predict year %d step %d lr %f'predict_year,i,config.lr)
                logging.info('%f %f %f %f %f %f %f %f %f',t_L,t_C,t_U,t_R,t_loss,np.mean(t_pred),np.mean(t_real),np.mean(t_pred-t_real),t_err)
                logging.info('%f %f %f %f %f %f %f %f %f',v_L,v_C,v_U,v_R,v_loss,np.mean(v_pred),np.mean(v_real),np.mean(v_pred-v_real),v_err)
            if i%10 == 0:
                # do validation
                pred = []
                real = []
                for j in range(image_validate.shape[0] / config.B):
                    ...
                    pred.append(np.squeeze(pred_temp))
                    real.append(np.squeeze(real_temp))
